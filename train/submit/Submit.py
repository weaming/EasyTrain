...
class Submit(object):
    def __init__(self, ticketDetails):
        ...
        self._urlInfo = (
            submitUrls["wc"]
            if self.__ticket.tourFlag == TourFlag.GO_BACK
            else submitUrls["dc"]
        )
    ...
    def _submitOrderRequest(self, tourFlag="dc"):
        formData = {
            ...
            "back_train_date": time.strftime(
                "%Y-%m-%d", time.localtime()
            ),  # query date:2017-12-31
            "tour_flag": tourFlag,
    ...
    def _getExtraInfo(self):
        def getRepeatSubmitToken(html):
            repeatSubmitToken = re.findall(
                r"var globalRepeatSubmitToken = '(.*)'", html
            )[0]
        ...
        def decodeTicketInfoForPassengerForm(html):
            ticketInfoForPassengerForm = re.findall(
                r"var ticketInfoForPassengerForm=(.*);", html
            )[0]
    ...
    def _checkOrderInfo(self, passengersDetails, seatType, ticketTypeCodes=1):
        formData = {
            "cancel_flag": self.__ticket.ticketInfoForPassengerForm["orderRequestDTO"][
                "cancel_flag"
            ]
            or "2",
            "bed_level_order_num": self.__ticket.ticketInfoForPassengerForm[
                "orderRequestDTO"
            ]["bed_level_order_num"]
            or "000000000000000000000000000000",
            "passengerTicketStr": TrainUtils.passengerTicketStrs(
                seatType, passengersDetails, ticketTypeCodes
            ),
    ...
    def _getQueueCount(self):
        formData = {
            ...
            "train_date": datetime.strptime(
                self.__ticket.ticketInfoForPassengerForm["queryLeftTicketRequestDTO"][
                    "train_date"
                ],
                "%Y%m%d",
            ).strftime("%b %a %d %Y 00:00:00 GMT+0800")
            + " (中国标准时间)",
            "train_no": self.__ticket.ticketInfoForPassengerForm[
            ...
            "train_location": self.__ticket.ticketInfoForPassengerForm[
                "train_location"
            ],
            "_json_att": "",
            "REPEAT_SUBMIT_TOKEN": self.__ticket.repeatSubmitToken,
        ...
        return (
            ...
            jsonRet["messages"],
            jsonRet["data"]["ticket"]
            if "data" in jsonRet and "ticket" in jsonRet["data"]
            else -1,
            jsonRet["data"]["count"]
            if "data" in jsonRet and "count" in jsonRet["data"]
            else -1,
        )
    ...
    def _confirmSingleOrGoForQueue(self, passengersDetails):
        formData = {
            "passengerTicketStr": TrainUtils.passengerTicketStrs(
                self.__ticket.seatType, passengersDetails, self.__ticket.ticketTypeCodes
            ),
            "oldPassengerStr": TrainUtils.oldPassengerStrs(passengersDetails),
            "randCode": "",
            "purpose_codes": self.__ticket.ticketInfoForPassengerForm["purpose_codes"],
            "key_check_isChange": self.__ticket.ticketInfoForPassengerForm[
                "key_check_isChange"
            ],
    ...
    def _payOrderInfo(self):
        ...
        headers = {"User-Agent": FIREFOX_USER_AGENT, "Connection": "keep-alive"}
    ...
    def submit(self):
        ...
        status, msg, submitStatus, errMsg = self._checkOrderInfo(
            passengersDetails, self.__ticket.seatType, self.__ticket.ticketTypeCodes
        )
        if not Utils.check(status, "checkOrderInfo: %s" % msg) or not Utils.check(
        ...
        if not Utils.check(
            status, "confirmSingleOrGoForQueue: %s" % msg
        ) or not Utils.check(
            submitStatus, "confirmSingleOrGoForQueue: %s" % errMsg or "订单信息提交失败！"
        ):
            return False
    ...
    def showSubmitInfoPretty(self):
        ...
        Log.v(
            ...
            % (
                totalTicketNum,
                Fore.YELLOW
                + "{}元".format(TrainUtils.submitTicketTotalCost(jsonTicketInfo))
                + Fore.RESET,
            )
        )
        return True
    ...
    def __waitForOrderId(self):
        ...
        while True:
            ...
            Log.w(
                "未出票，订单排队中...预估等待时间: %s 分钟" % (interval if interval <= 30 else "超过30")
            )
            if interval > 30:
                time.sleep(60)
            elif interval > 20:

...
class Login(object):
    ...
    def _passportRedirect(self):
        params = {"redirect": "/otn/login/userLogin"}
    ...
    def _uamtk(self):
        ...
        return (
            ...
            if "result_message" in jsonRet
            else "no result_message",
    ...
    def _uamauthclient(self, apptk):
        ...
        def isSuccess(response):
            return (
                ...
                if response and "result_code" in response
                else False
            )
    ...
    def _loginNormal(self, userName, userPwd):
        ...
        payload = {"username": userName, "password": userPwd, "appid": "otn"}
        jsonRet = EasyHttp.send(self._urlInfo["login"], data=payload)

        def isLoginSuccess(responseJson):
            return (
                0 == responseJson["result_code"]
                if responseJson and "result_code" in responseJson
                else False,
                responseJson["result_message"]
                if responseJson and "result_message" in responseJson
                else "login failed",
            )
    ...
    def _loginAsyncSuggest(self, userName, userPwd):
        ...
        def isSuccess(response):
            return (
                response["status"] and response["data"]["loginCheck"] == "Y"
                if "data" in response
                else False,
                response["data"]["otherMsg"]
                if "data" in response
    ...
    def isLogin(self):
        ...
        return (
            jsonRet["data"]["flag"]
            if jsonRet and "data" in jsonRet and "flag" in jsonRet["data"]
            else False
        )
